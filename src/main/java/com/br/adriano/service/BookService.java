...
public class BookService {
	...
	public BookResponse createNewBook(BookRequest request) {
		this.repository.findByAuthor(request.getAuthor()).ifPresent(p -> {throw new BookAuthorResourceExistingException("O author já existe "+ request.getAuthor());});
	...
	public BookResponse updateNameBook(Long id, BookRequest request) {
		Book book = this.repository.findById(id).orElseThrow(() -> new IdNotFoundException("O id informado não existe " + id));
	...
	public BookResponse findByCpf(String author) {
		...
		return this.repository.findByAuthor(author).orElseThrow(() -> new BookAuthorResourceNotFoundException("O Author informado não existe "+ author));
